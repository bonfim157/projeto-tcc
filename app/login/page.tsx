...
const ROLE_CARDS: { papel: Papel; icon: string; desc: string; border: string; delay: number }[] = [
  { papel: 'professor', icon: '🎓', desc: 'Docentes e\nEducadores',   border: '#1e3a8a', delay: 0   },
  { papel: 'gestao',    icon: '🏛️', desc: 'Administração\ne Direção', border: '#4c1d95', delay: 80  },
  { papel: 'aluno',     icon: '📖', desc: 'Alunos e\nEstudantes',     border: '#16a34a', delay: 160 },
]
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    await doLogin({ login: loginVal, senha })
  }

  // Dispara shake quando um erro novo aparece
  const prevError = useRef<string | null>(null)
  // Usar useEffect não é possível aqui sem importar — fazemos via efeito lateral seguro:
  // Se o error mudou para um valor não-nulo, agendamos o shake
      ...
      >
        {!papel ? (
          ...
          <>
            ...
            {/* Cards com stagger de entrada */}
            <div className="flex flex-col sm:flex-row gap-4 w-full max-w-2xl stagger-children">
              {ROLE_CARDS.map(({ papel: p, icon, desc, border, delay }) => (
                <button
                  key={p}
          ...
          <div className="w-full max-w-sm">
            ...
            <div className="mb-7 animate-fadeSlideDown">
            ...
            <div className="rounded-2xl p-7 bg-white shadow-[0_24px_64px_rgba(0,0,0,0.6)] animate-fadeSlideUp">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Erro com animação shake */}
                {error && (
                  <div
                    ref={errorRef}
                    className={`bg-red-50 border border-red-200 text-red-700 text-xs px-4 py-3 rounded-lg
                      ${shake ? 'animate-shake' : ''}`}
                    role="alert"
                    aria-live="assertive"
                  >
